...
# from scheduler.GOBI import MBRScheduler
...
def initalizeEnvironment(environment, logger):
	...
	# TMMTRScheduler -----error DAD
	# scheduler =TMMTRScheduler()
	...
	# MADMMT
	# scheduler =MADMMTRScheduler()
	# RLRMMTR
	# scheduler =RLRMMTRScheduler()
	# DQL
	# scheduler = DQLScheduler('energy_latency_'+str(HOSTS)) 
	#orginal gobi
	# scheduler = GOBIScheduler('energy_latency_'+str(HOSTS))
	#orginal gobi2
	scheduler = GOBI2Scheduler('energy_latency2_'+str(HOSTS))
	# GA
	# scheduler=GAScheduler('energy_latency_'+str(HOSTS))
	#scheduler = GOBI2Scheduler('energy_latency_'+str(HOSTS))

	# BMR*
	# scheduler = GOBI2Scheduler('energy_latency_r2_'+str(HOSTS))
	# BMR
	# scheduler = GOBIScheduler('energy_latency_r_'+str(HOSTS)) # baraye model jadid gobi --in model sakhteh shode va vazn hayash train shodan
	
	# scheduler = GOBI2Scheduler('energy_latency4_'+str(HOSTS)) # baraye model jadid gobi* --in model sakhteh shode va vazn hayash train shodan
