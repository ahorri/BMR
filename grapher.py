...
# SAVE_PATH = './outnewmodel1/'
SAVE_PATH = './pdf2/'

# modelinfo=['M2','M1_R','M1_2','M1','GOBI*','GA3','GA2','GA', 'DQL','LR-MMT', 'MAD-MC']
# modelinfo=['BMR2_4312','BMR2','BMR2-2215','BMR2-3114', 'BMR2-3232','BMR2-3322', 'BMR2-3331','BMR2-4411','BMR2-a']
# modelinfo=['BMR*','BMR','GOBI*','DQL','LR-MMT','MAD-MC','TMCR','RLRMMTR','LRMMTR','MADMCR','MADMMT']
modelinfo=['BMR*','BMR','GOBI*','DQL','LR-MMT','MAD-MC']
# modelinfo=['BMR','GOBI*','DQL','LR-MMT','MAD-MC']
# modelinfo=['Alpha','Beta','Gamma',]
# modelinfo=['LR-MMT', 'MAD-MC']
Models = modelinfo 
...
# Colors = ['red', 'blue', 'green', 'orange', 'magenta', 'pink','cyan', 'maroon', 'grey']
...
for ylabel in yLabelsStatic:
	...
	for model in Models:
		...
		if ylabel == 'Average RAM Utilization (%)':
			# edit kardammm
			d = np.array([(np.average(10*np.array(i['ram'])/(np.array(i['ram'])+np.array(i['ramavailable']))) if i != [] else 0) for i in stats.hostinfo]) if stats else np.array([0.])
			# d=d/100
			Data[ylabel][model], CI[ylabel][model] = np.sum(d), mean_confidence_interval(d)
		# Scheduler metrics
...
for ylabel in yLabelsStatic:
	...
	plt.ylim(min(values)/1.5, max(values)+statistics.stdev(values))
	...
	# print('\n================================')
	# print()
	# print(SAVE_PATH+'Bar-'+ylabel.replace(' ', '_')+".pdf")
...
for ylabel in yLabelsStatic:
	...
	for model in Models:
		...
		if ylabel == 'Average RAM Utilization (%)':
			# edit kardammm
			d = np.array([(np.average(1000*np.array(i['ram'])/(np.array(i['ram'])+np.array(i['ramavailable']))) if i != [] else 0) for i in stats.hostinfo]) if stats else np.array([0.])
			# d=d*100
			Data[ylabel][model], CI[ylabel][model] = d, mean_confidence_interval(d)
		# Scheduler metrics
		if ylabel == 'Scheduling Time (seconds)':
